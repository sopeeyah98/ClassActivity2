                ...
                client.get(MainActivity.this, api_url, params, new AsyncHttpResponseHandler() {
                    ...
                    public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                        ...
                        try {
                            ...
                            intent.putExtra("city", jsonObj.getString("name"));

                            JSONObject sysObj = jsonObj.getJSONObject("sys");
                            intent.putExtra("country", sysObj.getString("country"));

                            JSONObject weatherObj = jsonObj.getJSONArray("weather").getJSONObject(0);
                            intent.putExtra("description", weatherObj.getString("description"));

                            JSONObject mainObj = jsonObj.getJSONObject("main");
                            intent.putExtra("high", mainObj.getString("temp_max"));
                            intent.putExtra("low",  mainObj.getString("temp_min"));
                            intent.putExtra("feels",  mainObj.getString("feels_like"));
